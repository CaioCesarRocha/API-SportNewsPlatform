...
export class ChampionshipController {
  ...
  relegateChampionship = async (request: Request, response: Response): Promise<Response> => {
    try {
      const { championshipId, clubId } = request.body as {
        championshipId: number;
        clubId: string;
      ...
      await this.championshipService.relegateChampionship(championshipId, clubId);

      return response.status(200).json({ message: "Championship relegated successfully." });
    } catch(err) {
      console.error("error: ", err);
      return response.status(500).json({
        message: "Failed to relegate championship.",
      });
    }
  };
